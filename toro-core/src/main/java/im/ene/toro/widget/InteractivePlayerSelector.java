...
import static im.ene.toro.widget.Common.find;
import static java.util.Collections.singletonList;

/**
 * @author eneim (2018/08/17).
 *
 * A {@link OnLongClickListener} that co-operates with {@link Container} to selectively trigger the
 * playback. The common usecase is to allow user to long click on a {@link ToroPlayer} to trigger its
 * playback. In that case, we should set that {@link ToroPlayer} to highest priority among the
 * candidates, and also to clear its priority when user scroll it out of the playable region.
 *
...
public class InteractivePlayerSelector implements PlayerSelector, OnLongClickListener {

  protected final WeakReference<Container> container;
  ...
  public InteractivePlayerSelector(Container container, PlayerSelector delegate) {
    this.container = new WeakReference<>(ToroUtil.checkNotNull(container));
    this.delegate = ToroUtil.checkNotNull(delegate);
  }

  @Override public boolean onLongClick(View v) {
    Container ref = container.get();
    if (ref == null) return false;
    toPause.set(NO_POSITION); // long click will always mean to 'press to play'.
    RecyclerView.ViewHolder viewHolder = ref.findContainingViewHolder(v);
    boolean handled = viewHolder instanceof ToroPlayer;
    int position = handled ? viewHolder.getAdapterPosition() : -1;
    if (position != toPlay.getAndSet(position)) {
      ref.onScrollStateChanged(RecyclerView.SCROLL_STATE_IDLE);
    }
  ...
  @Override @NonNull public Collection<ToroPlayer> select(@NonNull Container container,
    ...
    // Make sure client doesn't miss use this instance to wrong Container.
    if (container != this.container.get()) return new ArrayList<>();

    if (toPause.get() > 0) {
      ToroPlayer toPauseCandidate = find(items, filterToPause);
    ...
    if (toPlay.get() >= 0) {
      ToroPlayer toPlayCandidate = find(items, filterToPlay);
  ...
  public void toPause(@SuppressWarnings("unused") int position) {
    ...
    Container ref = container.get();
    if (ref != null) ref.onScrollStateChanged(RecyclerView.SCROLL_STATE_IDLE);
  }
}
