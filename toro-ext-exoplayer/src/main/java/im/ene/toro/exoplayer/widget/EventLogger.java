...
/* package */ class EventLogger implements ExoMediaPlayer.Listener, ExoMediaPlayer.InfoListener,
  ...
  @Override public void onStateChanged(ToroExoPlayer player, boolean playWhenReady, int state) {
    Log.d(TAG, "state ["
  ...
  public void onLoadCompleted(int sourceId, long bytesLoaded, int type, int trigger, Format format,
    ...
    if (VerboseLogUtil.isTagEnabled(TAG)) {
      ...
      Log.v(TAG,
          "loadEnd [" + getSessionTimeString() + ", " + sourceId + ", " + downloadTime + "]");
    }
  }

  @Override public void onVideoFormatEnabled(Format format, int trigger, long mediaTimeMs) {
    Log.d(TAG,
        "videoFormat [" + getSessionTimeString() + ", " + format.id + ", " + Integer.toString(
            trigger) + "]");
  }

  @Override public void onAudioFormatEnabled(Format format, int trigger, long mediaTimeMs) {
    Log.d(TAG,
        "audioFormat [" + getSessionTimeString() + ", " + format.id + ", " + Integer.toString(
            trigger) + "]");
  }
