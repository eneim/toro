 ...
 */
@SuppressWarnings("unused") public abstract class VideoControls extends RelativeLayout {
  ...
  @NonNull protected Handler visibilityHandler = new Handler();
  @NonNull protected Repeater progressPollRepeater = new Repeater();

  @Nullable protected EMVideoView videoView;

  @Nullable protected VideoControlsSeekListener seekListener;
  @Nullable protected VideoControlsButtonListener buttonsListener;
  @Nullable protected VideoControlsVisibilityListener visibilityListener;

  @NonNull protected InternalListener internalListener = new InternalListener();
   ...
   * and the seek bar
   */
  public abstract void updateProgress(@IntRange(from = 0) long position,
      @IntRange(from = 0) long duration, @IntRange(from = 0, to = 100) int bufferPercent);
   ...
   * is loading.  This is different from using {@link #loadCompleted()} and {@link
   * #restartLoading()}
  ...
  public void setLoading(boolean isLoading) {
    ...
    textContainer.setVisibility(
        isLoading || (hideEmptyTextContainer && isTextContainerEmpty()) ? View.GONE : View.VISIBLE);
  }
   ...
   * as the third line of text and unlike the {@link #setTitle(CharSequence)} and {@link
   ...
   */
  public void setPlayPauseImages(@DrawableRes int playResourceId,
      @DrawableRes int pauseResourceId) {
   ...
   * of the button, if you want to change the enabled/disabled images see {@link
  ...
  @NonNull public List<View> getExtraViews() {
  ...
  protected void updateButtonDrawables() {
    defaultPlayDrawable =
        EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_play_arrow_white,
            R.color.exomedia_default_controls_button_selector);

    defaultPauseDrawable = EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_pause_white,
        R.color.exomedia_default_controls_button_selector);
    playPauseButton.setImageDrawable(defaultPlayDrawable);

    defaultPreviousDrawable =
        EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_skip_previous_white,
            R.color.exomedia_default_controls_button_selector);
    previousButton.setImageDrawable(defaultPreviousDrawable);

    defaultNextDrawable =
        EMResourceUtil.tintList(getContext(), R.drawable.exomedia_ic_skip_next_white,
            R.color.exomedia_default_controls_button_selector);
    nextButton.setImageDrawable(defaultNextDrawable);
  }
   ...
   */
  @SuppressWarnings("RedundantIfStatement") protected boolean isTextContainerEmpty() {
  ...
  protected void updateProgress() {
    if (videoView != null) {
      updateProgress(videoView.getCurrentPosition(), videoView.getDuration(),
  ...
  protected class InternalListener
    ...
    @Override public boolean onPlayPauseClicked() {
      if (videoView == null) {
        return false;
      }
    ...
    @Override public boolean onSeekStarted() {
      if (videoView == null) {
        return false;
      }
    ...
    @Override public boolean onSeekEnded(int seekTime) {
      if (videoView == null) {
        return false;
      }
