 ...
 */
@TargetApi(Build.VERSION_CODES.JELLY_BEAN) public class ExoMediaPlayer implements MediaPlayerApi {
  ...
  @Override public void setDataSource(@NonNull Context context, @Nullable Uri uri) {
    RenderBuilder builder =
        uri == null ? null : getRendererBuilder(MediaSourceUtil.getType(uri), uri);
    setDataSource(context, uri, builder);
  }

  @Override public void setDataSource(Context context, @Nullable Uri uri,
      @Nullable RenderBuilder renderBuilder) {
    if (uri == null) {
  ...
  @Override public void setVolume(@FloatRange(from = 0.0, to = 1.0) float left,
  ...
  /**
   * If the media has completed playback, calling {@code restart} will seek to the beginning of the
   * media, and play it.
   *
