...
public class PagingRequestHelper {
  ...
  @GuardedBy("mLock") private final RequestQueue[] mRequestQueues = new RequestQueue[] {  //
   ...
   */
  @AnyThread public boolean addListener(@NonNull Listener listener) {
  ...
  @GuardedBy("mLock") private StatusReport prepareStatusReportLocked() {
    Throwable[] errors = new Throwable[] {
        mRequestQueues[0].mLastError, mRequestQueues[1].mLastError, mRequestQueues[2].mLastError
    };
    return new StatusReport(getStatusForLocked(RequestType.INITIAL),
        getStatusForLocked(RequestType.BEFORE), getStatusForLocked(RequestType.AFTER), errors);
  }

  @GuardedBy("mLock") private Status getStatusForLocked(RequestType type) {
    return mRequestQueues[type.ordinal()].mStatus;
  }

  @SuppressWarnings("WeakerAccess") @AnyThread @VisibleForTesting //
  ...
  @FunctionalInterface public interface Request {
    ...
    class Callback {
       ...
       */
      @SuppressWarnings("unused") public final void recordSuccess() {
       ...
       */
      @SuppressWarnings("unused") public final void recordFailure(@NonNull Throwable throwable) {
        //noinspection ConstantConditions
        if (throwable == null) {
          throw new IllegalArgumentException(
              "You must provide a throwable describing" + " the error to record the failure");
        }
        if (mCalled.compareAndSet(false, true)) {
  ...
  public static final class StatusReport {
     ...
     */
    @Nullable public Throwable getErrorFor(@NonNull RequestType type) {
